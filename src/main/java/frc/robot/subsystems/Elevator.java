...
public class Elevator extends SubsystemBase {
  ...
  private final DynamicMotionMagicVoltage mm_request = new DynamicMotionMagicVoltage(0, 60, 120, 1200);

  private final double L1_inMotorRotations = 10;
  private final double L2_inMotorRotations = 20;
  private final double L3_inMotorRotations = 30;
  private final double L4_inMotorRotations = 40;
  ...
  public Elevator() {
    ...
    elevatorLeadMotor = new TalonFX(Constants.Elevator_Constants.elevatorLeadMotor, Constants.Elevator_Constants.kCanbus);
    elevatorFollowMotor = new TalonFX(Constants.Elevator_Constants.elevatorFollowMotor, Constants.Elevator_Constants.kCanbus);
  ...
  public Command setElevatorPositionCommand(double targetPositionInMotorTicks, double velocity, double acceleration, double jerk) {
    return this.runOnce( () -> setElevatorPosition(targetPositionInMotorTicks, velocity, acceleration, jerk));
  }

  public Command incrementElevatorPositionCommand(double incrementInMotorTicks) {
    return this.runOnce( () -> setElevatorPosition(elevatorLeadMotor.getPosition().getValueAsDouble() + incrementInMotorTicks));
  }

  public Command decrementElevatorPositionCommand(double decrementInMotorTicks) {
    return this.runOnce( () -> setElevatorPosition(elevatorLeadMotor.getPosition().getValueAsDouble() - decrementInMotorTicks));
  }
